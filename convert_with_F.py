...
def img_F_transform(img1, pts1, pts2, F, bd_box=(0,100,0,100), l=600, u_all = 0):
    ...
    start = time.time()
    print("=============================")
    print("Total number of pixels to calculate : ", total_pix)
    for r, row in enumerate(img1):
        for c, pix in enumerate(row):
            ...
            '''
            if count==100:
                sec = total_pix*(time.time()-start)//100
                result_list = str(datetime.timedelta(seconds=sec)).split(".")
                print("Estimated time = ", result_list)
            
            if count % (total_pix//100) == 0:
                percent = count // (total_pix//100)
                sec = (time.time()-start)
                result_list = str(datetime.timedelta(seconds=sec)).split(".")
                est_remain = ((total_pix-count)*sec)//count
                est_list = str(datetime.timedelta(seconds=est_remain)).split(".")
                print("Progress : ", percent,"%  / Has been running for ", result_list[0]," / Estimated remaining time : ", est_list[0], end='\r')
            b_new = est_pt(a, b, F)
...
def bd_box_tar2ref(image_ref, image_tar, reduce=0.3, meth = 'sift', bd_box_tar = [272, 484, 544, 847]):
    ...
    # bd box 크기 키우기
    w = int((bd_box_ref[2]-bd_box_ref[0])*0.2)
    h = int((bd_box_ref[3]-bd_box_ref[1])*0.2)
...
def get_proj_img(image_ref, image_tar, reduce=0.3, meth = 'sift', bd_box = (272, 484, 544, 847)):
    ...
    _, _ = draw(img1, img2, pts1, pts2, F, line = False, pt_sz=30)
    e = get_residual_error(F, pts1, pts2)
    print("number of interest points : ", pts1.shape[0])
    ...
    #print("bd : ", bd)
    for i in bd:
        _, b, _, _, _ = find_closeip_weight(i, pts1, pts2, len = 600, use_all = 1)
...
parser = argparse.ArgumentParser(description='Remove unwanted objects from the target image and restore the background using a different perspective photograph.\nThis code gets n, bd_yolo, method, scale as argparser. For example : python convert_with_F.py 3 300,400,240,560 sift 0.3 [Made by Team Gimoring]')    # 2. parser를 만든다.

# 3. parser.add_argument로 받아들일 인수를 추가해나간다.
parser.add_argument('n', help='Folder number ex) 3')    # 필요한 인수를 추가
parser.add_argument('bd_yolo', help='Boundary box in target image : x1,y1,x2,y2 ex) 300,400,240,560')
parser.add_argument('method', help='Feature extraction method ex) sift')
parser.add_argument('scale', help='Image resize scale for calculation ex) 0.3')

args = parser.parse_args()    # 4. 인수를 분석

n = args.n
reduce = float(args.scale)
...
cv2.imwrite('runs/F_results/'+method+'_F_result'+str(n)+'.jpg', cv2.cvtColor(target_new, cv2.COLOR_RGB2BGR))
